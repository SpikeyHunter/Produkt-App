	...
	<div class="flex-1 overflow-y-auto p-2 space-y-2 custom-scroll">
		...
		{:else}
			{#each filteredTrackers as item (item.id)}
				<div 
					class="group relative flex items-center gap-3 p-2 rounded-lg border-2 hover:border-gray2/30 cursor-pointer transition-all {selectedId === item.id ? 'border-lime bg-gray1/50' : 'border-transparent bg-gray1/50'}"
					on:click={() => handleSelect(item)}
					role="button"
                    tabindex="0"
                    on:keypress={(e) => { if(e.key === 'Enter') handleSelect(item); }}
				>
					<div class="w-12 h-16 bg-black rounded overflow-hidden flex-shrink-0">
						...
						{:else}
							<div class="w-full h-full flex items-center justify-center bg-gray2/20">
								<svg class="w-6 h-6 text-gray2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
							</div>
						{/if}
					</div>
					...
					<div class="flex-1 min-w-0">
						...
						<p class="text-xs text-lime">{formatDate(item.start_date)} - {formatDate(item.end_date)}</p>
					</div>
