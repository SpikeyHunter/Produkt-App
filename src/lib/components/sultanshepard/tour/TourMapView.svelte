<script lang="ts">
	...
	function getTypeColor(type?: string) {
		...
		return currentTypeColors[t] || currentTypeColors['Tour Date'] || '#E1FF00';
	}
	...
	async function saveLineColor(colorHex: string) {
		...
		await saveTourLineColor(colorHex);
	}
	...
	async function updateMapContent() {
		...
		const lineGeoJSON = { type: 'FeatureCollection', features: allRouteFeatures };
		...
		if (!eventsBound) {
			...
			map.on('click', (e) => {
				const features = map.queryRenderedFeatures(e.point, { layers: ['tour-variants-layer'] });
	...
	function showPopupForDate(data: any, lngLat: any) {
		...
		container.querySelector('#zoom-to-point')?.addEventListener('click', () => {
			map.flyTo({ center: lngLat, zoom: 12, speed: 3, curve: 1, essential: true });
		});
