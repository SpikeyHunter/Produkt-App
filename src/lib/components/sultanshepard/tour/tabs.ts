...
const ICONS = {
	event: 'M8 7V3m8 4V3M3 11h18M5 5h14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2z',
	budget: 'M12 1v22M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6',
	venue: 'M3 21h18M5 21V7l7-4 7 4v14M9 21v-6h6v6',
	production: 'M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z',
	setlist: 'M9 18V5l12-2v13M9 18a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm12-2a3 3 0 1 1-6 0 3 3 0 0 1 6 0z',
	logistics: 'M1 3h15v13H1zM16 8h4l3 3v5h-7V8zM5.5 21a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5zm13 0a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z',
	merch: 'M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4zM3 6h18M16 10a4 4 0 0 1-8 0',
	media: 'M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2zM12 17a4 4 0 1 0 0-8 4 4 0 0 0 0 8z',
	immigration: 'M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10zM12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20z',
	todo: 'M9 11l3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11',
	notes: 'M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z',
...
const TOUR_DATE_TABS: TabDef[] = [
	...
	{ id: 'show_budget', label: 'Show Budget', icon: ICONS.budget, restricted: true },
...
export function tabProgress(tab: TourDataTab, data: SSTourData, crew: SSCrew[] = []): number {
	switch (tab) {
		case 'event_details': {
			...
			const checks = [
				filled(d.status),
				filled(d.artist_entrance),
				filled(d.load_in_instructions),
				filled(d.parking_instructions),
				filled(d.contacts),
				filled(d.set_times),
				filled(d.crew_ids)
			];
		...
		case 'show_budget': {
			const d = data.show_budget || {};
			const checks = [
				d.flights?.enabled !== undefined,
				d.hotels?.enabled !== undefined,
				d.per_diem?.enabled !== undefined,
				d.transports?.enabled !== undefined
			];
			return pct(checks.filter(Boolean).length, checks.length);
		}
		case 'venue_info': {
			const d = data.venue_info || {};
			const checks = [
				filled(d.indoor_outdoor),
				filled(d.venue_type),
				d.shower !== undefined,
				d.green_room !== undefined,
				!d.green_room || filled(d.green_rooms)
			];
			return pct(checks.filter(Boolean).length, checks.length);
		}
		case 'production': {
			...
			const checks = [
				filled(d.artist_specs_status),
				d.artist_specs_confirmed === true,
				filled(d.stage_width) && filled(d.stage_depth),
				d.led_wall !== undefined,
				d.power_confirmed === true,
				filled(d.local_crew),
				filled(d.load_in_time),
				filled(d.load_out_time)
			];
			return pct(checks.filter(Boolean).length, checks.length);
		}
		case 'set_list':
			return (data.set_list?.songs?.length || 0) > 0 ? 100 : 0;
		case 'logistics': {
			const d = data.logistics || {};
			const checks = [
				filled(d.artist_hospo),
				filled(d.prod_hospo),
				filled(d.meal_mode),
				filled(d.transports),
				filled(d.guestlist_allocation)
			];
			return pct(checks.filter(Boolean).length, checks.length);
		}
		case 'merch': {
			const d = data.merch || {};
			if (d.enabled === false) return 100;
			if (d.enabled === undefined) return 0;
		...
		case 'media': {
			...
			const checks = [
				d.brief_sent === true,
				d.photo_link_received === undefined ? false : !d.photo_link_received || filled(d.photo_link),
				d.interviews_enabled !== undefined,
				d.meet_greet_enabled !== undefined
			];
			return pct(checks.filter(Boolean).length, checks.length);
		}
		case 'immigration': {
			const d = data.immigration || {};
			if (d.enabled === false) return 100;
			const rows = d.rows || [];
			if (!rows.length) return 0;
			const done = rows.filter((r) => r.letter_or_visa_received && r.sent_to_crew).length;
		...
		case 'travel': {
			...
			const done = people.filter((p) => p.booked && p.hotel_status && p.hotel_status !== 'not_booked').length;
