<script lang="ts">
	...
	let pixelMap = venueInfo.pixel_map;
	let camera = venueInfo.camera;
	let sdiConfirmed = venueInfo.sdi_confirmed;
	let bringOwnCamera = venueInfo.bring_own_camera;

	// FIX: Safely handle show_specs whether it's a string or a JSON object
	let specsLink = '';
	if (typeof show.show_specs === 'string') {
		specsLink = show.show_specs;
	} else if (show.show_specs && typeof show.show_specs === 'object') {
		// Handle the case where it is stored as { "url": "..." }
		specsLink = (show.show_specs as any).url || '';
	}
	...
	function handleLinkInput() {
		...
		debounceTimer = setTimeout(async () => {
			// Save as a plain string to fix the DB format moving forward
	...
	function cycleStatus(current: boolean | null): boolean | null {
		// Cycle: TBD (null) -> Yes (true) -> No (false) -> TBD (null)
	...
	function getStatusColor(status: boolean | null) {
		// Yes = Green (#86EFAC)
		// No = Red (#FCA5A5)
		// TBD = Yellow (#FDBA74) - "Proposed/TBD"
		if (status === true) return '#86EFAC';
		if (status === false) return '#FCA5A5';
		return '#FDBA74';
	}
	...
	async function handleUpload(e: CustomEvent) {
		...
		try {
			...
			const { error } = await supabase.storage
				.from('documents')
				.upload(`sultanshepard/${fileName}`, file);
			...
			{#if camera === true}
				<div class="flex justify-between items-center pl-3 border-l-4 border-lime animate-fade-in">
					...
					<button
						...
						on:click={() => {
							sdiConfirmed = cycleStatus(sdiConfirmed);
							saveVenueInfo();
						}}
					>
			...
			{:else if camera === false}
				<div class="flex justify-between items-center pl-3 border-l-4 border-lime animate-fade-in">
					...
					<button
						...
						on:click={() => {
							bringOwnCamera = cycleStatus(bringOwnCamera);
							saveVenueInfo();
						}}
					>
