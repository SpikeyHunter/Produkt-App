<script lang="ts">
	...
	export let message = 'Please confirm the changes to this event. Notifications will be sent to opted-in users.';
			...
			if (data) {
				...
				const parseValue = (params: any) => {
					if (typeof params === 'string') {
						try { return JSON.parse(params).value; } catch { return true; }
					}
					return params?.value ?? true;
				};

				if (emailSetting?.setting_params) globalDefaultEmail = parseValue(emailSetting.setting_params);
...
{#if show}
	...
	>
		...
		>
			<div class="w-12 h-12 rounded-full bg-question/20 text-question flex items-center justify-center mx-auto mb-4 shrink-0">
			...
			<div class="space-y-3 mb-8 text-left overflow-y-auto custom-scrollbar pr-2">
				{#if hasDateChange || hasTypeChange}
					<div class="p-4 bg-gray1/50 border border-gray2/20 rounded-xl space-y-3">
						<p class="text-sm font-bold text-white border-b border-gray2/20 pb-2">Detected Changes:</p>
						
						{#if hasDateChange}
							<div>
								<p class="text-[11px] font-bold text-gray2 uppercase tracking-wider">Date Changed</p>
								<div class="flex items-center gap-2 mt-1">
									<span class="text-sm font-medium text-problem line-through">{formatDateString(oldDates)}</span>
									<svg class="w-4 h-4 text-gray2" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
						...
						{#if hasTypeChange}
							<div>
								<p class="text-[11px] font-bold text-gray2 uppercase tracking-wider">Type Changed</p>
								<div class="flex items-center gap-2 mt-1">
				...
				{:else}
					<div class="p-4 bg-gray1/50 border border-gray2/20 rounded-xl text-center">
						<p class="text-sm font-bold text-gray2">No significant dates or type changes detected.</p>
					</div>
				{/if}
				...
				<div
					class="flex items-start gap-3 p-3 bg-gray1/50 border rounded-xl cursor-pointer transition-colors {optSendEmail ? 'border-question bg-question/5' : 'border-gray2/20 hover:bg-gray2/10'}"
				...
				>
					...
					<div class="flex-1">
						<p class="text-sm font-bold text-white leading-tight">
							Send Email notification to <span class="text-question">{emailUsersCount}</span> user{emailUsersCount !== 1 ? 's' : ''}
						</p>
					</div>
					{#if isAdmin}
						<button type="button" class="text-xs text-question font-bold hover:underline cursor-pointer" on:click|stopPropagation={() => dispatch('viewContacts')}>(view list)</button>
					{/if}
				</div>

				<div
					class="flex items-start gap-3 p-3 bg-gray1/50 border rounded-xl cursor-pointer transition-colors {optSendSms ? 'border-question bg-question/5' : 'border-gray2/20 hover:bg-gray2/10'}"
				...
				>
					...
					<div class="flex-1">
						<p class="text-sm font-bold text-white leading-tight">
							Send SMS notification to <span class="text-question">{smsUsersCount}</span> user{smsUsersCount !== 1 ? 's' : ''}
						</p>
					</div>
					{#if isAdmin}
						<button type="button" class="text-xs text-question font-bold hover:underline cursor-pointer" on:click|stopPropagation={() => dispatch('viewContacts')}>(view list)</button>
			...
			<div class="mt-auto flex gap-3 w-full shrink-0">
				<button class="flex-1 py-3 bg-transparent border border-gray2/20 text-gray2 font-bold rounded-2xl hover:bg-gray2/10 hover:text-white transition-colors cursor-pointer" on:click={handleCancel} disabled={saving}>Cancel</button>
				<button class="flex-[1.5] py-3 text-black/60 hover:text-black font-bold rounded-2xl bg-question hover:bg-question transition-colors flex justify-center items-center cursor-pointer" on:click={handleConfirm} disabled={saving}>
					{#if saving}<div class="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin mr-2"></div>{/if}
					Save & Notify
				</button>
			</div>
		</div>
