...
<div class="relative" bind:this={popupRef}>
	<button
		class="flex items-center gap-2 px-3.5 py-2.5 bg-navbar rounded-3xl hover:bg-white/5 transition-colors cursor-pointer"
	...
	>
		...
		<span class="text-xs font-black text-white"
			>{activeHolds.length} {event.status === 'CONFIRMED' ? 'Confirmed' : 'Holds'}</span
		>
		...
		>
			...
			<div class="grid grid-cols-7 gap-1.5 text-center mb-6">
				...
				{#each Array(daysInMonth) as _, i}
					...
					>
						{dayNum}
						{#if isSelected}<div
								class="w-1 h-1 rounded-full {event.status === 'CONFIRMED'
									? 'bg-confirmed'
			...
			<div class="flex gap-3 mt-2 border-t border-gray2/10 pt-4">
				...
				<button
					...
					on:click={promptModifyDates}>Confirm</button
				>
			</div>
		</div>
	{/if}
