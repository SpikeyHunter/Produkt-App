<script lang="ts">
	...
	$: uniqueRooms = Array.from(new Set(groupEvents.filter(e => e.status !== 'HIDDEN').map(e => `${e.venue.category}:::${e.venue.room}`)));

	$: displayLabel = (() => {
		if (uniqueRooms.length === 0) return 'Select Stage';
		const firstRoom = uniqueRooms[0].split(':::');
		if (uniqueRooms.length === 1) return `${firstRoom[0]} ${firstRoom[1]}`;
		return `${firstRoom[0]} ${firstRoom[1]} +${uniqueRooms.length - 1}`;
	})();
	...
	async function toggleVenue(vName: string, rName: string) {
		...
		if (isSelected) {
			...
			const idsToDelete = groupEvents.filter(e => e.venue.category === vName && e.venue.room === rName).map(e => e.id);
...
<div class="relative" bind:this={popupRef}>
	<button class="flex items-center gap-2 px-3 py-1 bg-navbar border border-gray2/20 rounded-full hover:bg-white/5 transition-colors shadow-sm cursor-pointer" on:click={() => showPopover = !showPopover}>
	...
	{#if showPopover}
		<div class="absolute left-0 top-[calc(100%+8px)] w-64 bg-navbar border border-gray2/20 rounded-xl shadow-xl overflow-hidden z-50 max-h-[350px] overflow-y-auto custom-scrollbar">
			<div class="p-3 border-b border-gray2/10 bg-gray1 sticky top-0 flex justify-between items-center">
				<span class="text-[10px] font-black text-gray2 uppercase tracking-widest">Select Venue(s)</span>
				<button class="text-[10px] font-bold text-lime hover:underline uppercase cursor-pointer" on:click={() => { showPopover = false; dispatch('openSettings', { venueId: null }); }}>+ Add</button>
			</div>
				...
				<div class="p-3 border-b border-gray2/10 last:border-0">
					<div class="flex justify-between items-center mb-2">
						...
						<button 
							...
							on:click={() => { showPopover = false; dispatch('openSettings', { venueId: venue.id }); }}
						>Edit</button>
					</div>
					...
					{#if stages && stages.length > 0}
						{#each stages as stage}
							...
							<label class="flex items-center gap-3 w-full py-1.5 px-2 rounded-lg hover:bg-white/5 transition-colors cursor-pointer {isSelected && uniqueRooms.length === 1 ? 'opacity-50 cursor-not-allowed' : ''}">
								<input type="checkbox" class="hidden" checked={isSelected} on:change={() => toggleVenue(venue.setting_name, stage.name)} disabled={isSelected && uniqueRooms.length === 1}>
								<div class="flex items-center justify-center w-4 h-4 rounded border {isSelected ? 'bg-lime border-lime' : 'border-gray2/50 bg-transparent'} transition-colors">
									{#if isSelected}<svg class="w-3.5 h-3.5 text-black" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>{/if}
								</div>
								<div class="flex items-center gap-2">
									<span class="w-2.5 h-2.5 rounded-full shadow-sm" style="background-color: {stage.color || '#ccc'}"></span>
									<span class="text-xs font-bold text-gray2 {isSelected ? 'text-white' : 'hover:text-white'} transition-colors">{stage.name}</span>
								</div>
							</label>
						{/each}
					{/if}
