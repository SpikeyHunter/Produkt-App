<script lang="ts">
	...
	$: filteredEvents = events
		.filter((e: any) => {
			...
			// LIVE / PAST split
			const isLive = e.event_status === 'LIVE';
			if (viewMode === 'LIVE' ? !isLive : isLive) return false;
...
<div class="event-selector-container flex flex-col gap-4 w-full h-full">
			...
			>
				<div class="p-2 border-b border-gray1 space-y-2">
					<div class="flex gap-1 bg-gray1 p-1 rounded-3xl">
						<button
							class="flex-1 text-xs font-bold py-1.5 cursor-pointer rounded-3xl transition-all {viewMode ===
							'LIVE'
								? 'bg-lime text-black shadow-sm'
								: 'text-gray-400 hover:text-white'}"
							on:click={() => (viewMode = 'LIVE')}>LIVE</button
						>
						<button
							class="flex-1 text-xs font-bold py-1.5 cursor-pointer rounded-3xl transition-all {viewMode ===
							'PAST'
					...
					{:else if filteredEvents.length > 0}
						{#each filteredEvents as event (event.event_id)}
							...
							>
								...
								{:else}
									<div
										class="w-12 h-16 bg-gray1 rounded flex items-center justify-center flex-shrink-0"
									>
										<svg
								...
								<div class="flex-1 min-w-0">
									<div
										class="text-white text-sm font-bold truncate transition-colors group-hover:text-lime"
									>
										{event.event_name}
									</div>
									...
									<div class="mt-1">
										<span
											...
											style="background-color: {statusInfo.color};">{statusInfo.label}</span
										>
									</div>
								</div>
								...
								{#if isSelected}
									<svg
										class="w-5 h-5 text-lime flex-shrink-0"
										viewBox="0 0 24 24"
										fill="none"
										stroke="currentColor"
										stroke-width="3"
					...
					{:else}
						<div class="p-4 text-center text-gray2 text-sm">
							{searchTerm
								...
								: `No ${viewMode.toLowerCase()} events available`}
						</div>
					{/if}
				</div>
			</div>
	...
	{#if selectedEvent}
		<div class="w-full bg-navbar border border-gray1 rounded-xl p-3 flex items-start gap-4">
			<div
				class="flex-shrink-0 w-24 rounded-lg overflow-hidden border border-gray1 relative self-start bg-black"
			>
				...
				{#if selectedEvent.event_status === 'LIVE'}
					<div
						class="absolute top-1 right-1 px-1.5 py-0.5 rounded text-[8px] font-bold uppercase bg-lime text-black shadow-sm"
					>
						LIVE
					</div>
				{/if}
	...
	{:else}
		<div
			class="w-full bg-navbar border border-gray1 rounded-xl p-6 flex flex-col items-center justify-center text-gray2 gap-2"
		>
			<svg
	...
	<div
		class="bg-navbar border border-gray1 rounded-xl p-3 transition-all duration-300 {selectedEvent
			? ''
			...
			{#each STATUS_OPTIONS as statusId}
				...
				<button
					...
					class="py-2.5 px-2 rounded-lg text-xs font-bold transition-all
						...
						: 'cursor-pointer hover:opacity-90 hover:border-gray3 hover:text-white'}"
	...
	<button
		...
		class="w-full py-2.5 rounded-3xl font-bold text-sm tracking-wide transition-colors cursor-pointer flex items-center justify-center gap-2 {isViewingAllEvents
			? 'bg-transparent border-2 border-lime text-lime hover:bg-lime hover:text-black'
			: 'bg-transparent border-2 border-gray2 text-gray2 hover:bg-gray2 hover:text-black'}"
	>
		...
		>
			...
			<div class="flex gap-3 justify-end">
				<button
					...
					on:click={() => (showApproveModal = false)}>Cancel</button
				>
				<button
					type="button"
					...
					on:click={handleApprove}>Confirm</button
				>
			</div>
		</div>
	</div>
