<script lang="ts">
    ...
    function getStatusColor(status: string) {
        ...
        switch(status.toLowerCase()) {
            case 'draft': return 'text-gray2 bg-gray1 border border-gray2/30';
            case 'confirmed': return 'text-tentatif bg-tentatif/10 border border-tentatif/30';
            case 'invoiced': return 'text-proposed bg-proposed/10 border border-proposed/30';
            case 'approved': return 'text-question bg-question/10 border border-question/30';
            case 'submitted': return 'text-info bg-info/10 border border-info/30';
            case 'paid': return 'text-confirmed bg-confirmed/10 border border-confirmed/30';
...
<button 
    ...
    {selected ? 'border-lime shadow-[0_0_0_2px_rgba(132,204,22,1)]' : 'border-gray1 hover:border-gray2 hover:shadow-lg'}"
    on:click={() => dispatch('click', artist)}
>
    <div class="absolute inset-0 bg-gray1 z-0">
        {#if artist.event_flyer}
            <img src={artist.event_flyer} alt="" class="w-full h-full object-cover opacity-30 group-hover:opacity-20 transition-opacity" />
    ...
    <div class="relative z-10 flex flex-col h-full p-4 justify-between">
        ...
        <div class="space-y-1.5">
             <h3 class="font-bold text-white text-lg leading-tight drop-shadow-md group-hover:text-lime transition-colors">
                {artist.artist_name}
            </h3>
        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="flex items-center justify-between">
                <span class="text-[10px] uppercase tracking-wider px-2 py-1 rounded-full font-bold {getStatusColor(artist.paymentData?.status || 'Draft')}">
