<script lang="ts">
	...
	$: {
        ...
        const filtered = events.filter((event) => {
            ...
            // if (!checkingAdvance && !advancedEventIds.has(event.event_id)) return false; 
    ...
    <div class="p-3 border-b border-gray1 space-y-3 bg-gray1/30 flex-shrink-0 z-10">
        ...
        <div class="bg-gray1 p-1 rounded-full flex gap-1">
            {#each timeFilterOptions as t}
                <button 
                    class="flex-1 py-1.5 text-[10px] font-bold rounded-full transition-all uppercase tracking-wide 
                    {timeFilter === t ? 'bg-lime text-black shadow-md' : 'text-gray2 hover:text-white'}"
    ...
    <div class="flex-1 overflow-y-auto p-2 scroll-smooth" bind:this={listContainer}>
        ...
        {:else}
            {#if (timeFilter === 'ALL' || timeFilter === 'LIVE') && liveEvents.length > 0}
                {#if timeFilter === 'ALL'}
                    <div class="px-2 py-2 mb-1 text-[14px] font-bold text-lime uppercase tracking-wider opacity-100  bg-navbar z-20 shadow-sm">
                        Upcoming
                    </div>
                {/if}
            ...
            {#if (timeFilter === 'ALL' || timeFilter === 'PAST') && pastEvents.length > 0}
                {#if timeFilter === 'ALL'}
                     <div class="px-2 py-2 mb-1 text-[14px] font-bold text-gray2 uppercase tracking-wider opacity-100 bg-navbar z-20 shadow-sm">
                        Past Events
                    </div>
                {/if}
