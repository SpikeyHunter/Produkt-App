<script lang="ts">
	...
	$: totalExpenses = totalArtistFee + totalTechnical + totalHospitality + totalOther;
...
>
	...
	<div class="mb-8">
		...
		<div class="grid grid-cols-2 gap-4">
			<div class="bg-gray2/10 p-4 rounded border border-gray2/20">
				...
				<span class="text-lime font-bold font-mono text-xl">{formatMoney(incomeArtist)}</span>
			</div>
			<div class="bg-gray2/10 p-4 rounded border border-gray2/20">
				<span class="text-gray2 text-xs uppercase block mb-1">Technical</span>
				<span class="text-lime font-bold font-mono text-xl">{formatMoney(incomeTechnical)}</span>
			</div>
			<div class="bg-gray2/10 p-4 rounded border border-gray2/20">
				<span class="text-gray2 text-xs uppercase block mb-1">Hospitality</span>
				<span class="text-lime font-bold font-mono text-xl">{formatMoney(incomeHospitality)}</span>
			</div>
			<div class="bg-gray2/10 p-4 rounded border border-gray2/20">
				<span class="text-gray2 text-xs uppercase block mb-1">Other</span>
				<span class="text-lime font-bold font-mono text-xl">{formatMoney(incomeOther)}</span>
			</div>
		</div>
	</div>
	...
	<div class="space-y-8 mb-8">
		...
		{#if budgetData?.artist_fee?.length > 0}
			<div>
				<h3 class="text-lime font-bold text-sm uppercase mb-3">Artist Fee</h3>
				<div class="bg-gray2/10 rounded-lg p-4 border border-gray2/20">
					...
					<div class="flex justify-between items-center pt-3 mt-2 border-t border-gray2/20 font-bold">
						...
						<span class="text-red-500 font-mono">{formatMoney(totalArtistFee * -1)}</span>
					</div>
				</div>
			</div>
		{/if}
		...
		{#if budgetData?.technical?.length > 0}
			<div>
				<h3 class="text-lime font-bold text-sm uppercase mb-3">Technical</h3>
				<div class="space-y-4">
		...
		{#if budgetData?.hospitality?.length > 0}
			<div>
				<h3 class="text-lime font-bold text-sm uppercase mb-3">Hospitality</h3>
				<div class="space-y-4">
		...
		{#if budgetData?.other_expenses?.length > 0}
			<div>
				<h3 class="text-lime font-bold text-sm uppercase mb-3">Other Expenses</h3>
				<div class="space-y-4">
	...
	<div class="mt-8 pt-6 border-t-2 border-gray2/30 break-inside-avoid">
		...
		<div class="bg-gray2/10 rounded-lg p-6 border border-gray2/20 space-y-3">
			<div class="flex justify-between items-center text-sm">
				<span class="text-gray2 uppercase tracking-wider">Total Income</span>
				<span class="font-bold text-lime text-lg font-mono">{formatMoney(totalIncome)}</span>
			</div>
			<div class="flex justify-between items-center text-sm">
				<span class="text-gray2 uppercase tracking-wider">Total Expenses</span>
				<span class="font-bold text-red-500 text-lg font-mono">{formatMoney(totalExpenses * -1)}</span>
			</div>
			<div class="border-t border-gray2/20 my-2"></div>
			<div class="flex justify-between items-center text-xl">
				<span class="font-bold text-white uppercase tracking-wider">Net Total</span>
				<span class="font-bold font-mono {netTotal >= 0 ? 'text-lime' : 'text-red-500'}">
...
<style>
	/* Print specific overrides to match AdvanceSheet logic */
	@media print {
		@page {
			size: 8.5in 11in;
			margin: 0;
