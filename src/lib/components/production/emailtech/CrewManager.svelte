            ...
            <div class="bg-gray1 rounded-lg p-3">
				...
				<div class="space-y-2">
					<input
						...
						class="w-full bg-navbar text-white rounded px-2 py-1.5 text-xs placeholder-gray2 focus:outline-none focus:ring-1 focus:ring-lime"
					/>
					<input
						type="email"
					...
					<div class="flex gap-2 pt-1">
						<button
							...
							class="flex-1 bg-lime text-black rounded px-3 py-1.5 text-xs font-bold hover:bg-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
						>
							Add
						</button>
						<button
							...
							class="flex-1 bg-gray2 text-black rounded px-3 py-1.5 text-xs font-bold hover:bg-white transition-colors cursor-pointer"
						>
							Cancel
						</button>
					</div>
		...
		{:else if filteredCrew.length > 0}
			<div class="space-y-2">
				{#each filteredCrew as member (member.id)}
					...
					>
						<div
							class="w-full bg-gray1 text-white rounded-lg p-2 transition-all duration-200 hover:bg-gray2 hover:text-black {draggedMember?.id ===
							member.id
								? 'opacity-50'
								: ''}"
						>
	...
	<div class="p-4 border-t border-gray1 flex-shrink-0 transition-all duration-200 
		...
		<div class="grid grid-cols-3 gap-2">
			{#each roleSlots as slot}
				...
				>
					<div
						class="bg-gray1 rounded-lg p-3 min-h-[80px] h-full flex flex-col items-center justify-center
...
<style>
	...
	::-webkit-scrollbar-track {
		background: #1a1a1a; /* bg-navbar */
	}
	::-webkit-scrollbar-thumb {
		background: #e1ff00; /* lime */
		border-radius: 3px;
	}
</style>