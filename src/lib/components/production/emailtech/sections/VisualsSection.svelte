        ...
        <div class="flex flex-col gap-6">
            ...
            {#if !isBazart}
                <div class="flex flex-col gap-1.5" transition:fly={{ y: -5, duration: 200 }}>
                    ...
                    {#if (isDSTRKT || isNCG)}
                        <div class="bg-gray1/20 border border-gray1 rounded-2xl p-3 text-xs text-gray3 font-mono leading-relaxed whitespace-pre-wrap select-text">
                ...
                <div class="relative sponsor-dropdown">
                    ...
                    >
                        <span class="flex items-center gap-2 truncate">
                            <span class="w-3 h-3 rounded-full flex-shrink-0" style="background-color: {currentSponsorColor};"></span>
                            <span>{currentSponsorLabel}</span>
                        </span>
                        
                        {#if !readOnly}
                            <svg class="w-4 h-4 text-gray2 transition-transform {showSponsorDropdown ? 'rotate-180' : ''}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
