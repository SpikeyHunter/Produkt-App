<script lang="ts">
	 ...
	 */
	function generateBacklineList(
		allEvents: any[],
		mainId: number | string | null,
		secondId: number | string | null
	) {
		if (!allEvents) {
			formData.backline = [];
			return;
	...
	function getVenueBackline(allEvents: any[], eventId: number | string, defaultVenueName: string) {
		...
		// NEW: Create a Set to store unique 'other' items to avoid duplicates
		...
		relevantEvents.forEach((row) => {
			...
			// NEW: Process 'other' items array
			if (rider.other && Array.isArray(rider.other)) {
				rider.other.forEach((otherItem: any) => {
					if (otherItem && otherItem.text) {
						otherItemsSet.add(otherItem.text.trim());
					}
				});
			}
		...
		// NEW: Add all collected 'other' items to the final list
		otherItemsSet.forEach((item) => {
		...
		if (mainId) {
			...
			if (mainArtists.length > 0) {
				...
				if (mainArtists[0].venue) {
					title =
						mainArtists[0].venue === 'New City Gas'
							? 'Main Room - Riders'
							: `${mainArtists[0].venue} - Riders`;
				}
...
>
	<div class="flex flex-col gap-4">
		{#if formData.backline && formData.backline.length > 0}
			...
			{/each}
		{:else}
			<div class="text-xs text-gray2 italic mt-2">No backline data available in Tech Riders.</div>
		{/if}
					...
					<div class="flex flex-col gap-2">
						...
						<div class="flex flex-col gap-2">
							{#each group.artists as artist}
								...
								>
									<svg
										class="w-4 h-4 text-lime flex-shrink-0"
										fill="none"
										stroke="currentColor"
										viewBox="0 0 24 24"
									>
