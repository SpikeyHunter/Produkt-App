...
<div class="relative datepicker-container {width} {height}" bind:this={containerElement}>
    ...
    {#if showDatePicker}
        <div
            class="absolute top-full left-0 mt-1.5 bg-navbar border border-lime rounded-xl shadow-2xl z-[9999] p-4 w-80"
            role="dialog"
            aria-modal="true"
        >
            <div class="flex items-center justify-between mb-4">
                <button
                    ...
                    class="p-2 text-gray2 hover:text-white !hover:cursor-pointer hover:bg-gray1 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-lime"
            ...
            <div class="grid grid-cols-7 gap-1 mb-2">
                {#each ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'] as day}
                    <div class="text-center text-sm text-gray2 font-medium py-2">
                        {day}
                    </div>
                {/each}
            </div>
            ...
            <div class="grid grid-cols-7 gap-1">
                {#each calendarDays as day}
                    <button
                        type="button"
                        class="aspect-square text-base transition-colors cursor-pointer rounded-lg flex items-center justify-center h-9 w-9 {day.isCurrentMonth
                            ? day.isSelected
