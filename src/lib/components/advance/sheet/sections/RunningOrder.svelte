<script lang="ts">
  ...
  // Determine if this artist should be highlighted
  function isHighlightedArtist(artistName: string): boolean {
    // Check if this is the event's main artist
    if (event.artist_name && artistName.toLowerCase() === event.artist_name.toLowerCase()) {
      return true;
    }
    // Also check notes for "Headliner" designation
    const entry = timetable.find(t => t.artist === artistName);
    if (entry?.notes?.toLowerCase().includes('headliner')) {
      return true;
    }
    return false;
  }
