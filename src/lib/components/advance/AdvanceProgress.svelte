<script lang="ts">
	...
	function handleFieldUpdate(e: CustomEvent) {
		...
		if (event.event_id === eventId && event.artist_name === artistName) {
			...
			} else if (column === 'tech_rider_status') {
				event.tech_rider_status = value;
			} else if (column === 'hospitality_rider_status') {
				event.hospitality_rider_status = value;
			}
		}
	}
	...
	// Check event.timetable first (which gets updated immediately), then fall back to mainEvent.timetable
	$: rosConfirmedStatus = (() => {
	...
	$: flightsStatus = (() => {
		...
		const hasCompleteArrival = groundInfo.arrivals?.some(
			(arrival: any) =>
				arrival.date && arrival.time && arrival.flightNumber && arrival.from && arrival.to
		);
		const hasCompleteDeparture = groundInfo.departures?.some(
			(departure: any) =>
	...
	function getRiderBadgeColor(status: string): string {
		const normalizedStatus = status ? status.toLowerCase().trim() : 'waiting';
		switch (normalizedStatus) {
			case 'confirmed':
				return 'bg-confirmed text-black'; // Green
			case 'to do':
				return 'bg-tentatif text-black'; // Yellow/Orange
			case 'waiting':
				return 'bg-problem text-black'; // Red
			default:
				return 'bg-gray2 text-black'; // Gray fallback
		}
	}
</script>
...
<div
	...
	style="width: 280px; height: 425px;"
>
	...
	<div class="px-6 py-2 h-full">
		<div class="px-2 py-1 flex flex-col gap-3">
			...
			{:else}
				...
				<div class="flex items-center gap-3 text-sm">
					<span class="font-semibold min-w-[120px] text-gray3">Tech Rider</span>
					<DropdownButton
						{event}
						options={['Waiting', 'To Do', 'Confirmed']}
						value={event.tech_rider_status || 'Waiting'}
						column="tech_rider_status"
						valueType="text"
						buttonClass={getRiderBadgeColor(event.tech_rider_status || 'Waiting')}
						on:fieldUpdate={handleFieldUpdate}
					/>
				</div>

				<div class="flex items-center gap-3 text-sm">
					<span class="font-semibold min-w-[120px] text-gray3">Hospitality Rider</span>
					<DropdownButton
						{event}
						options={['Waiting', 'To Do', 'Confirmed']}
						value={event.hospitality_rider_status || 'Waiting'}
						column="hospitality_rider_status"
						valueType="text"
						buttonClass={getRiderBadgeColor(event.hospitality_rider_status || 'Waiting')}
						on:fieldUpdate={handleFieldUpdate}
					/>
				</div>
			{/if}
