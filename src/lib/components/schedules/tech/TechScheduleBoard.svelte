<script lang="ts">
	...
	async function createRow(date: string, sortOrder: number) {
		...
		if (!error && data) {
			rows = [...rows, data]; // Instant UI Update
	...
	async function handleKeydown(e: KeyboardEvent) {
		...
		const canEditField = (field: string | null) =>
			field &&
			(field !== '__ROW__' || userPermissions.canEditAll) &&
			(userPermissions.canEditAll || userPermissions.allowedColumns.includes(field));

		if ((key === 'c' || key === 'x') && targetField) {
			e.preventDefault();
			clipboardData =
				targetField === '__ROW__'
					? { type: 'row', data: { ...row } }
					: { type: 'cell', data: (row as any)[targetField], field: targetField };
			if (key === 'x' && canEditField(targetField)) {
		...
		if (key === 'v' && clipboardData) {
			e.preventDefault();
			if (targetField === '__ROW__' && clipboardData.type === 'row' && userPermissions.canEditAll) {
			...
			} else if (targetField && clipboardData.type === 'cell' && canEditField(targetField)) {
				updateCell(row.id, targetField, clipboardData.data);
			}
		}
	}
	...
	async function handleMenuAction(event: CustomEvent) {
		...
		switch (action) {
			...
			case 'duplicate':
				...
				const { data: newDup } = await supabase
					.from('schedule_techs')
					.insert({ ...rest, sort_order: newOrder })
					.select()
					.single();
				if (newDup) {
					rows = [...rows, newDup]; // Optimistic Add
				}
				break;
			case 'delete':
			...
			case 'clear':
				if (targetField === '__ROW__' && userPermissions.canEditAll) {
					...
					// FIX: Prevent clearing date column with empty string
