<script lang="ts">
	...
	export let remotePresences: Record<string, { user: string; color: string; field: string; rowId: string }> = {};
	...
	$: hasBackground = row.type && row.type !== 'Hold' && row.type !== 'Canceled' && EVENT_COLORS[row.type] && EVENT_COLORS[row.type] !== 'transparent';
	$: bgStyle = row.type === 'Canceled' ? 'background-color: transparent;' : `background-color: ${EVENT_COLORS[row.type] || 'transparent'};`;
	$: textColorStyle = (() => {
	...
	const cellBaseClass = 'w-full h-full bg-transparent px-1 border-0 outline-none text-[14px] placeholder-gray-500 font-normal leading-tight disabled:cursor-default disabled:text-opacity-80 transition-shadow duration-200';
    ...
    function getTypeStyle(type: string) {
        ...
        if (type === 'Other' || type === 'Bazart') return `background-color: ${type === 'Bazart' ? '#BDBDBB' : 'transparent'}; color: ${type === 'Bazart' ? 'black' : '#BDBDBB'}; border: ${type === 'Bazart' ? '1px solid #BDBDBB' : 'none'};`;
...
<div role="row" tabindex="0" style="{gridStyle}; font-family: Arial, Helvetica, sans-serif;" class="border-b border-gray2/10 hover:bg-white/5 transition-colors min-h-[24px] items-stretch relative {isSelected ? 'bg-lime/10' : ''}" on:keydown>
	...
	<button type="button" class="w-full h-full text-center text-gray2 select-none {stdBorder} justify-center cursor-context-menu hover:text-white focus:text-white focus:outline-none bg-transparent text-[14px]" on:click={(e) => dispatch('contextmenu', { e, row, field: '__ROW__' })} on:contextmenu={(e) => handleContextMenu(e, '__ROW__')} disabled={userPermissions.role === 'viewer'}>
		{index + 1}
	</button>

	<div class="px-2 select-none {stdBorder} justify-center text-[14px] {isWeekend ? 'bg-black/20 text-gray3 !font-medium' : 'text-gray3 !font-medium'}" on:contextmenu={(e) => handleContextMenu(e, 'date')} role="gridcell" tabindex="0">
		{dayjs(row.date).format('dddd')}
	</div>

	<div class="{rangeBorder} h-full {isWeekend ? 'bg-black/20' : ''}" style="{getPresenceStyle('date')}" title="{getPresenceTooltip('date')}">
		<input type="text" value={dateInput} readonly={!isEditable('date')} on:focus={() => handleFocus('date')} on:blur={handleDateBlur} on:contextmenu={(e) => handleContextMenu(e, 'date')} class="{cellBaseClass} text-center cursor-text text-gray3 !font-medium" />
	</div>

	<div class="{stdBorder} h-full justify-center px-1 relative" on:contextmenu={(e) => handleContextMenu(e, 'type')} role="gridcell" tabindex="-1">
		<button bind:this={typeButtonRef} type="button" disabled={!isEditable('type')} on:click={toggleTypeDropdown} on:contextmenu={(e) => handleContextMenu(e, 'type')} class="text-[12px] h-[20px] px-3 rounded-full uppercase tracking-wide truncate transition-transform hover:scale-105 active:scale-95 flex items-center justify-center min-w-[110px] disabled:opacity-70 disabled:hover:scale-100 disabled:cursor-not-allowed {isEditable('type') ? 'hover:cursor-pointer' : ''}" style={getTypeStyle(row.type)}>
			{row.type || 'Select'}
		</button>
	</div>

	<div class="{rangeBorder} h-full p-[2px]" style="{getPresenceStyle('event_name')}" title="{getPresenceTooltip('event_name')}">
		<textarea 
            use:autoResize 
            rows="1" 
	...
	<div class={stdBorder} style="{getPresenceStyle('op_hours')}" title="{getPresenceTooltip('op_hours')}">
		<input 
    ...
    <div class={rangeBorder} style="{getPresenceStyle('crew_call')}" title="{getPresenceTooltip('crew_call')}">
		<input 
            class="{cellBaseClass} text-center" 
	...
	<div class={rangeBorder} style="{getPresenceStyle('artist_liaison')}" title="{getPresenceTooltip('artist_liaison')}">
		<input 
            class="{cellBaseClass} text-center" 
            value={activeField === 'artist_liaison' ? editBuffer : (row.artist_liaison || '')} 
            readonly={!isEditable('artist_liaison')} 
            style={getStaffFieldStyle(activeField === 'artist_liaison' ? editBuffer : row.artist_liaison, row.type)} 
...
{#if showTypeDropdown}
	<div class="fixed z-[9999] bg-[#1e1e1e] border border-gray-700 rounded-lg shadow-2xl py-1 max-h-[300px] overflow-y-auto flex flex-col min-w-[160px]" style="top: {dropdownPosition.top}px; left: {dropdownPosition.left}px;" role="menu" tabindex="-1" on:click|stopPropagation on:keydown>
		<div class="px-2 py-1 text-[10px] text-gray3 font-bold uppercase tracking-wider mb-1">Select Event Type</div>
		{#each EVENT_TYPES as type}
			<button class="px-3 py-1.5 text-[13px] text-left hover:bg-white/10 hover:cursor-pointer flex items-center gap-2 transition-colors w-full group" on:click={() => selectType(type)}>
				<div class="w-3 h-3 rounded-full border border-white/10 shadow-sm" style="background-color: {EVENT_COLORS[type] === 'transparent' ? '#333' : EVENT_COLORS[type]}; {type === 'Hold' ? 'border:1px dashed #888' : ''}"></div>
				<span class="text-gray-300 font-medium group-hover:text-white transition-colors {type === 'Canceled' ? 'text-red-400 line-through' : ''}">{type}</span>
			</button>
		{/each}
		<div class="h-px bg-gray-700 my-1 mx-2"></div>
		<button class="px-3 py-1.5 text-[12px] text-left text-problem hover:cursor-pointer hover:text-red-300 hover:bg-red-500/10 transition-colors w-full" on:click={() => selectType('')}>Clear Selection</button>
	</div>
{/if}