<script lang="ts">
  ...
  let dateInput = dayjs(row.date).format('D MMM'); 
  let dateRef: HTMLInputElement;
  $: if (row.date && !document.activeElement?.isSameNode(dateRef)) {
  ...
  function handleStaffBlur(e: Event, field: keyof TechRow) {
      ...
      if (/^n[\/\\]?a$/i.test(val)) val = 'N/A';
      else val = val.toLowerCase().replace(/\b\w/g, (char) => char.toUpperCase());
      if (val !== row[field]) {
  ...
  function autoResize(node: HTMLTextAreaElement) {
    ...
    resize();
    return { destroy() { node.removeEventListener('input', resize); }, update() { resize(); } };
  }
...
<div 
  ...
  class="border-b border-gray2/10 hover:bg-white/5 transition-colors min-h-[24px] items-stretch {isSelected ? 'bg-lime/10' : ''}"
  on:keydown
>
  <button 
    type="button"
  ...
  <div 
    class="px-2 select-none {stdBorder} justify-center text-[14px] {isWeekend ? 'bg-black/20 text-gray3 !font-medium' : 'text-gray3 !font-medium'}"
        ...
        {#each EVENT_TYPES as type}
            ...
            >
                <div 
                    ...
                    style="background-color: {EVENT_COLORS[type] === 'transparent' ? '#333' : EVENT_COLORS[type]}; {type==='Hold'?'border:1px dashed #888':''}"
                ></div>
                <span class="text-gray-300 font-medium group-hover:text-white transition-colors {type === 'Canceled' ? 'text-red-400 line-through' : ''}">
                    {type}
                </span>
            </button>
        {/each}
