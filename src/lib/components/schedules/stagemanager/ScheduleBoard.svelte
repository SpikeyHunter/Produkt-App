<script lang="ts">
    ...
    let modalData: { staff: Staff, weekId: number, dayIdx: number, shift: Partial<Shift>, dayShiftCount: number, existingShifts: Shift[] } | null = null;
    ...
    function setupRealtime() {
        ...
        realtimeChannel = supabase
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'schedule_weeks' }, (payload) => handleWeekChange(payload))
            .on('postgres_changes', { event: '*', schema: 'public', table: 'schedule_shifts' }, (payload) => handleShiftChange(payload))
    ...
    async function reloadShiftsOnly() {
         ...
         const { data: shiftsData } = await supabase.from('schedule_shifts').select('*').in('week_id', weekIds);
         if(shiftsData) {
             weeks = weeks.map(week => ({
                ...week,
    ...
    async function loadWeeksAndShifts() {
        const { data: weeksData } = await supabase.from('schedule_weeks').select('*').order('start_date', { ascending: true });
        if (weeksData) {
    ...
    async function addStaffToWeek(weekId: number, staffId: number) {
        ...
        const placeholder = { week_id: weekId, staff_id: staffId, day_index: 0, start_time: '00:00', end_time: '00:00', shift_type: 'PLACEHOLDER' as any };
    ...
    function openModal(weekId: number, staff: Staff, dayIdx: number, shift: Shift | null, currentShiftsInDay: number) {
        if (!currentUser) return;
    ...
    function handleKeyDown(e: KeyboardEvent, shift: Shift | undefined, weekId: number, staffId: number, dayIdx: number) {
        if (!currentUser) return; 
        if ((e.ctrlKey || e.metaKey) && e.key === 'c' && shift) { copiedShift = { ...shift }; delete copiedShift.id; e.preventDefault(); }
        if ((e.ctrlKey || e.metaKey) && e.key === 'v' && copiedShift) {
            e.preventDefault();
            const wIdx = weeks.findIndex(w => w.id === weekId);
            if (wIdx >= 0) {
                const count = weeks[wIdx].shifts.filter(s => s.staff_id === staffId && s.day_index === dayIdx && (s.shift_type as string) !== 'PLACEHOLDER').length;
                if (count >= 2) return; 
            }
        ...
        if ((e.key === 'Delete' || e.key === 'Backspace') && shift?.id) { handleModalDelete({ detail: shift.id } as CustomEvent); }
    }
</script>
...
<div class="h-full w-full p-6 text-white overflow-y-auto">
    ...
    <div class="flex flex-col gap-12 pb-20">
        {#each sortedWeeks as week (week.id)}
            ...
            <div class="bg-gray2/5 rounded-2xl p-6 border border-gray2/10 relative group/week transition-all hover:border-gray2/20">
                <div class="flex justify-between items-start mb-6">
                    <h2 class="text-2xl font-bold text-lime">Week of {getWeekRangeString(week.start_date)}</h2>
                                    ...
                                    <div class="absolute right-0 top-full mt-2 w-64 bg-gray1 border border-gray2 rounded-xl shadow-2xl z-30 flex flex-col overflow-hidden" use:clickOutside on:click_outside={() => activeAddWeekId = null}>
                                        ...
                                        <div class="max-h-60 overflow-y-auto">
                                            {#each filteredStaffList(week.id) as s}
                                                <button class="w-full text-left px-4 py-3 hover:bg-lime/10 hover:text-lime text-sm border-b border-gray2/5 last:border-0 transition-colors cursor-pointer" on:click={() => addStaffToWeek(week.id, s.id)}>{s.name}</button>
                                            {:else}
                    ...
                    <table class="w-full min-w-[900px] border-collapse bg-black/20 table-fixed">
                        <thead>
                            <tr class="bg-gray2/10">
                                <th class="text-left p-3 text-xs font-bold text-gray2 uppercase tracking-wider w-32 sticky left-0 bg-[#1a1a1a] z-10 border-r border-gray2/10">Staff</th>
                                {#each DAYS_FULL as day, i}
                                    <th class="text-center p-3 text-xs font-bold text-gray2 uppercase tracking-wider border-l border-gray2/10">
                                        <span class="hidden lg:inline">{day}</span>
                                        <span class="lg:hidden">{DAYS_SHORT[i]}</span>
                                    </th>
                                {/each}
                                <th class="text-center p-3 text-xs font-bold text-gray2 uppercase tracking-wider border-l border-gray2/10 w-20">Total</th>
                                {#if currentUser} <th class="w-12 p-0 border-l border-gray2/10"></th> {/if}
                            </tr>
                        </thead>
                        <tbody>
                            {#each rows as row}
                                <tr class="border-t border-gray2/10 hover:bg-white/5 transition-colors group/row">
                                    <td class="p-3 font-medium text-white sticky left-0 bg-[#1a1a1a] group-hover/row:bg-[#252525] z-10 border-r border-gray2/10 truncate">{getFirstName(row.staff.name)}</td>
                                    {#each row.shifts as dayShifts, i}
                                        <td class="p-1 border-l border-gray2/10 h-14 relative group/cell align-top" tabindex="0">
                                            <div class="w-full h-full flex flex-col gap-1">
                                                {#if dayShifts.length > 0}
                                                    {#each dayShifts as shift}
                                                        <div 
                                                            ...
                                                            on:click|stopPropagation={() => openModal(week.id, row.staff, i, shift, dayShifts.length)}
                                                            on:keydown={(e) => { if(e.key==='Enter') openModal(week.id, row.staff, i, shift, dayShifts.length); handleKeyDown(e, shift, week.id, row.staff.id, i); }}
                                                            role="button"
                                                            tabindex="0"
                                                        >
                                                ...
                                                {:else if currentUser}
                                                    <div 
                                                        ...
                                                        on:keydown={(e) => e.key === 'Enter' && openModal(week.id, row.staff, i, null, 0)}
                                                        role="button"
                                                        tabindex="0"
                                                    >
                                                        <div class="w-6 h-6 rounded-full bg-gray2/20 flex items-center justify-center text-gray2 hover:bg-lime hover:text-black transition-colors">+</div>
                                                    </div>
                                                {/if}
                                            </div>
                                        </td>
                                    {/each}
                                    <td class="p-3 text-center border-l border-gray2/10 font-mono text-lime font-bold text-sm">{formatHours(row.totalHours)}</td>
                                    {#if currentUser}
                                        <td class="p-0 border-l border-gray2/10 text-center">
                                            {#if confirmDeleteRow?.weekId === week.id && confirmDeleteRow?.staffId === row.staff.id}
                                                <button class="w-full h-14 flex items-center justify-center bg-red-500/20 text-red-500 hover:bg-red-500 hover:text-white transition-colors cursor-pointer" on:click={() => deleteRow(week.id, row.staff.id)} title="Confirm Remove"><svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg></button>
                                            {:else}
                                                <button class="w-full h-14 flex items-center justify-center text-gray2 hover:text-red-500 opacity-0 group-hover/row:opacity-100 transition-all cursor-pointer" on:click={() => confirmDeleteRow = { weekId: week.id, staffId: row.staff.id }} title="Remove Staff"><svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
                                            {/if}
                                        </td>
                                    {/if}
                                </tr>
                            ...
                            <tr class="bg-gray2/10 font-bold border-t-2 border-gray2/20">
                                <td class="p-3 text-gray2 text-right sticky left-0 bg-[#1f1f1f] z-10 border-r border-gray2/10 text-xs">TOTALS</td>
                                {#each dailyTotals as total} <td class="p-3 text-center text-gray2 border-l border-gray2/10 text-sm">{total > 0 ? formatHours(total) : '-'}</td> {/each}
                                <td class="p-3 text-center text-lime border-l border-gray2/10 text-sm">{formatHours(rows.reduce((acc, r) => acc + r.totalHours, 0))}</td>
        ...
        {:else}
            <div class="flex flex-col items-center justify-center py-20 text-gray2">
                ...
                {#if currentUser && viewMode === 'current'} <button class="text-lime hover:underline mt-2 font-bold" on:click={createNextWeek}>Create one now</button> {/if}
            </div>
        {/each}
    </div>
