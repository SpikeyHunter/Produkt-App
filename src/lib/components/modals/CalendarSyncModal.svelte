<script lang="ts">
	...
	async function toggleGroundEnabled() {
		...
		isSaving = true;
		try {
			const newGroundEnabled = !groundEnabled;
			await updateEventAdvance(event.event_id, event.artist_name, {
				ground_enabled: newGroundEnabled
			});

			event = { ...event, ground_enabled: newGroundEnabled };

			console.log(`Ground transport enabled toggled to: ${newGroundEnabled}`);
		} catch (error) {
			console.error('Failed to toggle ground enabled:', error);
		} finally {
			isSaving = false;
		}
	}
...
>
	<div class="relative">
		<div class="p-4 bg-gray1 rounded-lg {!groundEnabled ? 'opacity-20 blur-sm' : ''}">
		...
		{#if !groundEnabled}
			<div class="absolute inset-0 bg-black/60 backdrop-blur-sm rounded-lg flex items-center justify-center cursor-not-allowed">
				<div class="text-white text-center pointer-events-none">
					<p class="text-lg font-semibold">Ground Transport Disabled</p>
					<p class="text-sm text-gray2">Enable ground transport below to use this feature</p>
				</div>
			</div>
		{/if}
	</div>
	...
	<div slot="footer" class="flex justify-between items-center w-full p-4">
		<div class="flex items-center gap-3">
			<button
				...
				class={`flex items-center gap-2 px-4 py-3 rounded-full text-sm font-bold transition-all ${
					...
					syncButtonDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-90 cursor-pointer'
				}`}
			>
				{#if isSyncing}
		...
		<div class="flex items-center gap-3">
			<button
				type="button"
				on:click={toggleGroundEnabled}
				disabled={isSaving}
				class="px-8 py-3 text-sm border transition-colors rounded-full cursor-pointer {groundEnabled
