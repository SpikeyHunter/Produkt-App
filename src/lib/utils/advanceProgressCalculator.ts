...
export function calculateAdvanceProgress(event: EventAdvance): number {
	...
	if (hotelEnabled) {
		...
		console.log('Hotels enabled - including in progress calculation:', hotelProgress + '%');
	} else {
	...
	if (flightsEnabled) {
		...
		console.log('Flights enabled - including in progress calculation:', flightsProgress + '%');
	} else {
		console.log('Flights disabled - excluding from progress calculation');
	}
	...
	// 10. Ground Transport - EXCLUDE if ground_enabled is false
	const groundEnabled = event.ground_enabled !== false; // Default to true if null
	if (groundEnabled) {
		...
		console.log('Ground transport enabled - including in progress calculation');
	} else {
...
function calculatePassportProgress(event: EventAdvance): { value: number; included: boolean } {
	...
	rolesRequiringPassport.forEach((role: any) => {
		const hasCompletePassport = passports.some((p: any) => {
			...
			const hasAllFields =
				p.passportNumber && p.givenName && p.lastName && p.dateOfBirth && p.country;
	...
	console.log(
		`Passport progress: ${completedPassports}/${rolesRequiringPassport.length} = ${percentage}%`
	);
...
function calculateImmigrationProgress(event: EventAdvance): { value: number; included: boolean } {
	...
	console.log(
		`Immigration docs progress: ${completedImmigration}/${rolesRequiringImmigration.length} = ${percentage}%`
	);
...
function calculateGroundTransportProgress(event: EventAdvance): {
	value: number;
	included: boolean;
} {
