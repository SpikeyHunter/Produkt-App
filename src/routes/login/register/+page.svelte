<script lang="ts">
    ...
    // Permissions State
    let userPermissions = {
        role: 'viewer', // 'production' | 'editor' | 'booker' | 'viewer'
        canAddYear: false,
        canEditAll: false,
        allowedColumns: [] as string[] // empty means all if canEditAll is true
    };
	...
	// Default to hidden previous months
	...
	onMount(async () => {
		...
		// Ensure we default to today's year if available
    ...
    async function fetchUserPermissions(userId: string) {
        ...
        const roles = [data.main_permission, data.secondary_permission];
        
        // Priority Logic: Production > TechEditor > TechBooker
        if (roles.includes('Production')) {
            ...
            userPermissions = {
                ...
                canAddYear: false,
                canEditAll: false,
                // Maps to: LD, VIDEO, VJ, SOUND, TECH, DT, NOTES
	...
	async function fetchYears() {
		const { data } = (await supabase.from('schedule_techs').select('year')) as {
