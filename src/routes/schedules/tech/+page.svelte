		...
		{#if currentUser}
			<MainLayout>
				<div class="flex flex-col h-[calc(100vh-30px)] w-full bg-gray1">
					<slot name="content">
						<div
							class="flex items-center justify-between px-2 py-4 border-b border-gray2/20 bg-gray1"
						>
							<div class="flex items-center gap-6">
								...
								<div class="flex space-x-1">
									...
									{#if displayedYears.length === 0}
										<span class="text-gray2 text-xs italic py-2 px-2"
											>No {viewMode} years found</span
										>
									{/if}
								</div>
							</div>
							...
							<div class="flex items-center gap-4">
								<div
									class="text-[10px] uppercase font-bold tracking-widest px-2 transition-all duration-300"
								>
									{#if saveStatus === 'saving'}
										<span class="text-lime animate-pulse">Saving...</span>
									{:else if saveStatus === 'success'}
										<span class="text-lime">Saved</span>
									...
									>
										...
										{:else}
											<div class="relative w-5 h-5 flex items-center justify-center">
												...
												<div
													class="absolute inset-0 rounded-full border border-gray2 opacity-50"
												></div>
											</div>
										{/if}
									</button>
								...
								<button
									class="flex items-center gap-2 px-3 py-1.5 rounded-2xl border transition-all text-xs font-bold hover:cursor-pointer {hidePastMonths
								...
								>
									{#if hidePastMonths}
										...
										>
											<path
												fill-rule="evenodd"
												d="M3.707 2.293a1 1 0 00-1.414 1.414l14 14a1 1 0 001.414-1.414l-1.473-1.473A10.014 10.014 0 0019.542 10C18.268 5.943 14.478 3 10 3a9.958 9.958 0 00-4.512 1.074l-1.78-1.781zm4.261 4.26l1.514 1.515a2.003 2.003 0 012.45 2.45l1.514 1.514a4 4 0 00-5.478-5.478z M12.454 16.697L9.75 13.992a4 4 0 01-3.742-3.742L2.335 6.578A9.98 9.98 0 00.458 10c1.274 4.057 5.065 7 9.542 7 .847 0 1.669-.105 2.454-.303z"
												clip-rule="evenodd"
											/>
										</svg>
									{:else}
...
8.201 2.665 9.336 6.41.147.481.147.99 0 1.472C18.201 14.335 14.257 17 10 17c-4.257 0-8.201-2.665-9.336-6.41zM14 10a4 4 0 11-8 0 4 4 0 018 0z"
								...
								{#if currentUser}
                                    ...
                                    {userPermissions.canAddYear
											? 'hover:bg-lime/80 hover:cursor-pointer'
											: 'opacity-30 cursor-not-allowed grayscale'} 
                                    disabled:opacity-50 disabled:cursor-not-allowed"
										on:click={addYear}
										disabled={creatingYear || !userPermissions.canAddYear}
										title={userPermissions.canAddYear
											? 'Add a new year schedule'
											: 'Production role required'}
									>
