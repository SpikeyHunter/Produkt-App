	...
	<div class="h-full overflow-hidden p-6">
		<div class="control-center-container fade-in {mounted ? 'mounted' : ''}">
			...
			<div class="carousel-column">
				<EventCarouselManager bind:events={carouselEvents} on:eventsLoaded={handleEventsLoaded} on:select={handleSelectEvent} on:addFromSource={handleAddEvent} />
			</div>
	...
	{#if adding}
		<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
			<div class="bg-navbar border-2 border-lime rounded-xl p-6 flex flex-col items-center gap-3">
				<div class="animate-spin w-8 h-8 border-3 border-lime border-t-transparent rounded-full"></div>
