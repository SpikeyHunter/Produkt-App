<script lang="ts">
 import { onMount } from 'svelte';
  ...
  /**
   * Filters the `allEvents` array into the `events` array based on
   * the `event_status` property.
   */
  function filterEventsByStatus(): void {
    if (showLive) {
      // Live events are those with the status 'live'.
      events = allEvents.filter(event => event.event_status === 'LIVE');
    } else {
      // Past events are those with the status 'past'.
      events = allEvents.filter(event => event.event_status === 'PAST');
    }
    console.log(`Filtered for ${showLive ? 'LIVE' : 'PAST'} events. Found ${events.length} events.`);
  }

  /**
   * Handles the click event for the Live/Past toggle button.
   */
        ...
        <div class="controls-container">
          ...
          <div class="buttons-container">
            ...
            <div class="buttons-left">
              ...
              <button 
                class="h-7 px-4 flex items-center justify-center rounded-[14px] cursor-pointer transition-all duration-200 ease-in-out max-w-[50px] font-bold text-sm leading-[22px] {showLive ? 'bg-transparent border border-lime text-lime hover:!bg-lime hover:text-black' : 'bg-transparent border border-gray3 text-gray3 hover:!bg-gray3 hover:text-black'}"
