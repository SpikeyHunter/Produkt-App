<script lang="ts">
	...
	function getStatusButtonStyle(status: string, selected: boolean) {
		let base =
			'px-3 py-1.5 text-[10px] font-bold rounded-full border transition-all uppercase tracking-wide ';
		if (!selected) {
			let colors = '';
	...
	async function loadInitialData() {
		...
		if (urlStatus && availableStatuses.includes(urlStatus)) {
			selectedStatuses = [urlStatus];
		}

		// Logic fix: Ensure event is forcefully loaded if an event_id exists in URL,
		// regardless if mode is ALL or EVENT. We set currentEventId immediately.
		if (urlEventId) {
			...
			if (foundEvent) {
				...
				if (urlArtistName) {
					...
					const foundArtist = masterArtistList.find(
						(a) => a.artist_name === decodedName && a.event_id === foundEvent.event_id
					);
					if (foundArtist) selectedArtist = foundArtist;
				}
			}
		} else if (urlArtistName) {
			// Mode is ALL and no event ID is selected
			const decodedName = decodeURIComponent(urlArtistName);
						...
						<div class="flex flex-col gap-3">
							...
							<div class="flex flex-wrap gap-2">
								{#each availableStatuses as status}
									<button
										class="cursor-pointer transition-transform hover:scale-105 {getStatusButtonStyle(
											status,
					...
					<div class="flex-1 overflow-y-auto p-4">
						...
						{:else}
							{#if (timeFilter === 'ALL' || timeFilter === 'LIVE') && liveArtists.length > 0}
								...
								{/if}
								<div
									class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-4 mb-8"
								>
