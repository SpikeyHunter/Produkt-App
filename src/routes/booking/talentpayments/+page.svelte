<script lang="ts">
	...
	async function handleEventSelect(e: CustomEvent, shouldUpdateUrl = true) {
        ...
        if (partialEvent) {
            ...
            if (shouldUpdateUrl) updateUrl(partialEvent.event_id, null, 'EVENT', timeFilter, selectedStatuses.length > 0 ? selectedStatuses[0] : null);
		} else {
            selectedEvent = null;
            if (shouldUpdateUrl) updateUrl(null, null, viewMode, timeFilter, selectedStatuses.length > 0 ? selectedStatuses[0] : null);
        }
	}
	...
	async function handleArtistSelect(artist: any) {
        ...
        updateUrl(eId, selectedArtist.artist_name, viewMode, timeFilter, selectedStatuses.length > 0 ? selectedStatuses[0] : null);
	}
	...
	function updateUrl(eventId: number | null, artistName: string | null, mode: string, filter: string, status: string | null = null) {
...
<svelte:head>
	<title>{selectedArtist?.artist_name ? `${selectedArtist.artist_name} - Invoice DJ` : 'Talent Payments'}</title>
</svelte:head>
